...
FROM country 
WHERE country
IN (SELECT country FROM country WHERE country like '%Afghanistan%' or country like '%Bangladesh%' or country like '%China%'); 
...
SELECT store, CONCAT('$', total_sales) total_sales
FROM sales_by_store; 
...
CREATE VIEW top_rev_by_genre AS 
SELECT name, SUM(amount) as `Gross Revenue`
FROM category c 
JOIN film_category fc 
ON (c.category_id = fc.category_id)
...
SELECT * FROM top_rev_by_genre; 

-- 8c. You find that you no longer need the view `top_five_genres`. Write a query to delete it.

DROP VIEW top_rev_by_genre; 